...
;
let data = readdirSync('./DB/Images/Cards/')
...
const UPDATE_DB =()=>{
    for (let i = 0; i < base.length; i++) {
        ...
        do {
            ...
            if(!existsSync(`./DB/Filtrated/${base[i]}_${tri}.png`) && exceptions.indexOf(`${base[i]}_${tri}`) == -1){
                try {
                    console.log(`Trying ${base[i]}_${tri}.png`);
                    copyFileSync(`./DB/Images/Cards/${base[i]}_${tri}.png` , `./DB/Filtrated/${base[i]}_${tri}.png`)
                    updated++
